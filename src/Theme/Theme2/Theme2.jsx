...
const Theme2 = (props) => {
    ...
    const { name, address, phone, email, profile, profileImage, summary, skill, hobbie } = themeData.personalData;

    const { checkProj, checkWork, checkAward, checkHobbie,checkCertificate } = useContext(ResumeContext)
            ...
            <Box id="theme2">
                ...
                <section className='bottom-part d-flex mt-3'>
                    ...
                    <section className='partition-2'>
                        ...
                        {!checkHobbie &&
                            <Box className='mt-5'>
                                <Heading fontSize='2xl' className='my-2'>Hobbies</Heading>
                                <Box className='mt-3'>
                                    {
                                        hobbie?
                                        hobbie.split(',').map((item, index) => {
                                            return (
                                                <div key={index}>
                                                    <Badge className='mx-1' key={index}>{item}</Badge><br />
                                                </div>
                            ...
                            !checkCertificate &&
                            <Box id='experience-area'>
                                <Heading fontSize='2xl' className='mt-4'>Certificates</Heading>
                                {
                                    Object.entries(certificateTitles).map((element, index) => {
                                        return (
                                            <Box key={index} className="mt-1">
                                                ...
                                                <Box className='sub-details'>
                                                    {
                                                        (Object.entries(certificateDesc)[index] === undefined)
                                                            ?
                                                            null
                                                            :
